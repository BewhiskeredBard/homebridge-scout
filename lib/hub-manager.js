    ...
    values.forEach((serviceValues, serviceType) => {
        ...
        for (let characteristicType of serviceValues.keys()) {
            service.getCharacteristic(characteristicType).on('get', (callback) => {
                ...
                try {
                    value = this.getCharacteristicValues(accessory).get(serviceType).get(characteristicType);
                } catch (e) {
                    return callback(e);
                }
...
HubManager.prototype.getCharacteristicValues = function(accessory) {
    ...
    let alarmedModeId = modeIds.find(modeId => 'alarmed' == this.modeStates.get(modeId));
    let armedModeId = modeIds.find(modeId => -1 < ['armed', 'triggered'].indexOf(this.modeStates.get(modeId)));
    let armingModeId = modeIds.find(modeId => 'arming' == this.modeStates.get(modeId));
...
HubManager.prototype.setTargetState = async function(accessory, value) {
    if (3 == value) {
        let targetModeId = accessory.context.modeIds.find(modeId => {
