...
ScoutPlatform.prototype.getLocation = async function() {
    ...
    this.logger(`Using "${this.locationName}" location [${location.id}].`);
...
ScoutPlatform.prototype.getModeIds = function(modes) {
    ...
    keys.forEach(key => {
        ...
        this.logger(`Using "${modeName}" mode [${modeId}] for ${key} state.`);
...
ScoutPlatform.prototype.registerDevice = function(locationDevice) {
    this.logger(`Discovered "${locationDevice.name}" ${locationDevice.type} device [${locationDevice.id}].`);
